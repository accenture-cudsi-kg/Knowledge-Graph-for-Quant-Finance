...
XGB_classifier_file_name = "resources/XGB_template_classifier.pkl"
XGB_template_classifier = pickle.load(open(XGB_classifier_file_name, "rb"))

# "rb" because we want to read in binary mode
with open("resources/allennlp_relations_embeddings.pkl", "rb") as f:
    allennlp_relation_embeddings = pickle.load(f)

# "rb" because we want to read in binary mode
with open("resources/allennlp_entities_embeddings.pkl", "rb") as f:
    allennlp_entity_embeddings = pickle.load(f)


# loading ontologies
with open("resources/allennlp_ontology.json", 'r') as f:
...
# "rb" because we want to read in binary mode
with open("resources/stanford_relations_embeddings.pkl", "rb") as f:
    stanford_relation_embeddings = pickle.load(f)

# "rb" because we want to read in binary mode
with open("resources/stanford_entities_embeddings.pkl", "rb") as f:
    stanford_entity_embeddings = pickle.load(f)


# loading ontologies
with open("resources/stanfordopenie_ontology.json", 'r') as f:
...
bert_template = {4: ["SELECT (count(?o) as ?count) WHERE {<http://crypto.org/ENTITY_CLASS/ENTITY> <http://crypto.org/RELATION_CLASS/RELATION> ?o.}",
                     "SELECT (count(?s) as ?count) WHERE {?s <http://crypto.org/RELATION_CLASS/RELATION> <http://crypto.org/ENTITY_CLASS/ENTITY>.}"], 5: ["SELECT ?o WHERE { <http://crypto.org/ENTITY_CLASS/ENTITY> <http://crypto.org/RELATION_CLASS/RELATION> ?o. } ORDER BY AGGREGATION_METHOD(?o) LIMIT 1"]}
...
max_superlatives = ["highest", "maximum", "most",
                    "greatest", "best", "farthest", "longest"]
min_superlatives = ["least", "smallest", "lowest",
                    "minimum", "worst", "closest", "shortest"]

with open("resources/stanford_triples.json", 'r') as f:
    stanford_triples = json.load(f)
with open("resources/allennlp_triples.json", 'r') as f:
...
def get_single_relation(query, ontology):
    ...
    if max(query_relation_score) > 0.80:
        relation = relations[query_relation_score.index(
            max(query_relation_score))]
...
def map_entity(extracted_entity, ontology):
    ...
    if max(entity_onto_score) > 0.80:
        onto_entity = entities_ontology[entity_onto_score.index(
            max(entity_onto_score))]
        return onto_entity, ontology["entities"][onto_entity]
    else:
        return "Not found", "Not found"
